...
<div id="content-wrapper" class="d-flex flex-column">
    ...
    <div id="content">
        ...
        <div class="container-fluid">
            ...
            {
                // Gửi yêu cầu tới API endpoint trên cùng một máy chủ
                $response = file_get_contents('http://localhost:5000/get_user_count');
                return json_decode($response, true)['user_count'];
            }
            ?>

<p>Số người truy cập trực tiếp: <span id="user-count">0</span></p>

            <script src="https://cdnjs.cloudflare.com/ajax/libs/socket.io/4.0.1/socket.io.js"></script>
            <script>
                document.addEventListener('DOMContentLoaded', (event) => {
                    var socket = io('http://localhost:5000');  // Địa chỉ của server Flask
                    socket.on('user_count', function (count) {
                        document.getElementById('user-count').innerText = count;
                    });

                    // Nếu muốn hiển thị số người truy cập ban đầu khi trang load
                    fetch('http://localhost:5000/get_user_count')
