...
if (isset($_POST['thembtn'])) {
    if (isset($_POST['id']) && isset($_POST['ten'])) {
        ...
        $idtt = $_POST['id'];
        $ten = $_POST['ten'];
        $sql = "INSERT INTO tinhthanh (id, ten, id_nguoitao, ngaytao) VALUES ('$idtt', '$ten', '2', current_timestamp())";
...
if (isset($_POST['luubtn'])) {
    ...
    $sql = "UPDATE tinhthanh
        SET id='$idtt', ten = '$ten', id_nguoisua = '3', ngaysua = current_timestamp()
        WHERE id = '$idtt'";
...
if (isset($_POST['xoabtn'])) {
    ...
    $sql = "DELETE FROM `tinhthanh` WHERE id = '$idtt'";
    $stmt = $conn->prepare($sql);
    $query = $stmt->execute();
