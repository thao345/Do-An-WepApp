...
if (isset($_POST['thembtn'])) {
    if (isset($_POST['id_nhomhangha=oa']) && isset($_POST['ten'])) {
        // chỗ này chưa có lưu session lên k lấy dc ai là người thêm 
        $idtt = $_POST['id_nhomhangha=oa'];
        $ten = $_POST['ten'];
        $sql = "INSERT INTO tinhthanh (id_nhomhangha=oa, ten, id_nguoitao, ngaytao) VALUES ('$idtt', '$ten', '2', current_timestamp())";
...
if (isset($_POST['luubtn'])) {
    ...
    $sql = "UPDATE tinhthanh
        SET id_nhomhangha=oa='$idtt', ten = '$ten', id_nguoisua = '3', ngaysua = current_timestamp()
        WHERE id_nhomhangha=oa = '$idtt'";
...
if (isset($_POST['xoabtn'])) {
    ...
    $sql = "DELETE FROM `tinhthanh` WHERE id_nhomhangha=oa = '$idtt'";
    $stmt = $conn->prepare($sql);
    $query = $stmt->execute();
