...
if (isset($_POST['thembtn'])) {
    if (isset($_POST['id_']) && isset($_POST['ten'])) {
        // chỗ này chưa có lưu session lên k lấy dc ai là người thêm 
        $idtt = $_POST['id_'];
        $ten = $_POST['ten'];
        $sql = "INSERT INTO donvicungcapdau (id_, ten, id_nguoitao, ngaytao) VALUES ('$idtt', '$ten', '2', current_timestamp())";
...
if (isset($_POST['xoabtn'])) {
    ...
    $sql = "DELETE FROM `donvicungcapdau` WHERE id_ = '$idtt'";
    $stmt = $conn->prepare($sql);
    $query = $stmt->execute();
