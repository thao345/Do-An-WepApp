...
$sqlFilter = "SELECT nhansu.id_auto_increment, id_auto_increment,nhansu.id_nhansu, nhansu.ten, nhansu.tenphongban,nhansu.chucvu, nhansu.nguyenquan, nhansu.diachithuongtru, nhansu.ngaysinh, nhansu.cmnd, nhansu.sđt, nhansu.stk,nhansu.tennganhang, nguoidung.tendangnhap AS nguoitao, nguoidung.id_nguoidung, DATE(nhansu.ngaytao) as ngaytao, nguoidung2.tendangnhap AS nguoisua, nguoidung2.id_nguoidung, DATE(nhansu.ngaysua) as ngaysua
                    FROM nhansu 
                    INNER JOIN nguoidung ON nhansu.id_nguoitao = nguoidung.id_nguoidung
                    LEFT JOIN nguoidung AS nguoidung2 ON nhansu.id_nguoisua = nguoidung2.id_nguoidung";
if (!empty($nguoithuchien)) {
    $sqlFilter .= "  AND nhansu = '$nguoithuchien'";
}

if (!empty($from_date)) {
    // Thêm điều kiện tìm kiếm theo từ ngày
    $sqlFilter .= " AND nhansu.ngaytao >= '$from_date'";
}

if (!empty($to_date)) {
    // Thêm điều kiện tìm kiếm theo đến ngày
    $sqlFilter .= " AND nhansu.ngaytao <= '$to_date'";
}
