...
if (isset($_POST['thembtn'])) {
    ...
    $sql = "INSERT INTO tintuc ( tieude,special, noidung, img,ngaytao, id_nguoitao) 
                VALUES ( '$tieude','$special','$noidung','$img', current_timestamp(),'$id_nguoidung')";
    ...
    if (move_uploaded_file($img_tmp, '../Pacific/images/' . $img)) {
        if ($query) {
...
if (isset($_POST['luubtn'])) {
    ...
    if ($_FILES["img"]["name"] != '') {
        ...
        move_uploaded_file($img_tmp, '../Pacific/images/'.$img); // Di chuyển file mới vào thư mục ảnh
    } else {
    ...
    //     $image = $result[0]['img'];
    //  } else {
    //    $image = $_FILES['img']['name']; // lấy tên file mới 
    //    $image_tmp = $_FILES['img']['tmp_name']; // đường dẫn tạm thời
    //    move_uploaded_file($img_tmp, '../Pacific/images/'.$img); // di chuyển đường dẫn mới vào folder img
    //  }
  
    $noidung = $_POST["noidung"];
  
  
    $sql = "UPDATE  tintuc SET  tieude='$tieude', special='$special',  noidung='$noidung', img='$img',
...
if (isset($_POST['xoabtn'])) {
    $id_tintuc = $_POST['delete_id'];
     ...
     $sql1 = "UPDATE `tintuc` SET `id_nguoisua`='$id_nguoidung' WHERE id_tintuc = '$id_tintuc'";
